...
CHINESE_DECISION_CUE_CATEGORIES = {
    ...
    "negative-boundary": re.compile(
        r"(?:未必|不覆盖|不代表|不等于|不等同于|不保证|不适用|不成立|"
        r"仅(?=反映|适用|支持|覆盖|用于|保留|识别|估计|衡量|说明))"
    ),
    "causal-warning": re.compile(
        r"(?:会|将|可能|易|足以)(?:改变|破坏|导致|影响|造成|引发|扭曲|"
        r"放大|削弱|掩盖|遗漏)"
    ),
    ...
    "evidential": re.compile(
        ...
        r"决定(?=能否|是否|排序|稳定性|精度|误差|结果|结论|选择|性能|成本|"
        r"收敛|可靠性|敏感性|可识别性|适用性|方差|响应|风险|偏差))"
    ),
}
